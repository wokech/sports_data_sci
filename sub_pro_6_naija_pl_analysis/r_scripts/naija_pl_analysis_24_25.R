# Nigeria PL League Table Analysis (2024-2025)
...
str(naija_pl_merge_24_25)
head(naija_pl_merge_24_25)

# 1) Plot of Points and Goal Differences in a Bar Chart

naija_pl_merge_bar_24_25_pt_gd <- naija_pl_merge_24_25 |>
  select(team_name, GD, P)

team_order <- naija_pl_merge_bar_24_25_pt_gd %>%
  arrange(desc(P), desc(GD)) %>%
  pull(team_name)

naija_pl_merge_bar_24_25_pt_gd_long <- naija_pl_merge_bar_24_25_pt_gd |>
  pivot_longer(cols = c(P, GD), names_to = "metric", values_to = "value")

# Plot

naija_pl_merge_bar_24_25_pt_gd_long %>%
  mutate(team_name = factor(team_name, levels = rev(team_order))) %>%
  ggplot(aes(x = team_name, y = value, fill = metric)) +
  geom_col(position = position_dodge()) +
  geom_text(data = naija_pl_merge_bar_24_25_pt_gd,
...
naija_pl_merge_lollipop_24_25_w_l <- naija_pl_merge_24_25 |>
...
ggplot(naija_pl_merge_lollipop_24_25_w_l) +
...
naija_pl_merge_bar_24_25_ppg <- naija_pl_merge_24_25 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
  select(team_name, PPG) 

ggplot(naija_pl_merge_bar_24_25_ppg) +
...
naija_pl_merge_bar_24_25_gpg <- naija_pl_merge_24_25 |>
...
naija_pl_merge_bar_24_25_gapg <- naija_pl_merge_24_25 |>
  mutate(GAPG = round(A/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GAPG) 

ggplot(naija_pl_merge_bar_24_25_gapg) +
...
naija_pl_merge_bar_24_25_gdpg <- naija_pl_merge_24_25 |>
  mutate(GDPG = round(GD/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GDPG) 

ggplot(naija_pl_merge_bar_24_25_gdpg) +
...
naija_pl_merge_bar_24_25_gf_ga <- naija_pl_merge_24_25 |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
  select(team_name, F, A)

# Thresholds
against_thresh <- median(naija_pl_merge_bar_24_25_gf_ga$A)
...
naija_pl_merge_bar_24_25_ppg_gd <- naija_pl_merge_24_25 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
...
# Thresholds
goal_diff_thresh <- median(naija_pl_merge_bar_24_25_ppg_gd$GD)
ppg_thresh <- median(naija_pl_merge_bar_24_25_ppg_gd$PPG)
...
naija_pl_merge_24_25_non_perc <- naija_pl_merge_24_25 |>
  mutate(
    PPG = P / GP,
    GPG = F / GP,
    GAPG = A / GP,
...
naija_pl_merge_24_25_perc <- naija_pl_merge_24_25 |>
  mutate(
    WinPerc = (W / GP) * 100,
    DrawPerc = (D / GP) * 100,
    LossPerc = (L / GP) * 100,
...
naija_pl_merge_24_25_non_perc_radar <- naija_pl_merge_24_25_non_perc |>
  select(team_name, PPG, GPG, GAPG, GF2GA, GDPG, GD2P) 

naija_pl_merge_24_25_perc_radar <- naija_pl_merge_24_25_perc |>
...
naija_pl_merge_24_25_perc_radar_top_3 <- naija_pl_merge_24_25_perc_radar |>
...
perc_radar_top_3 <- ggradar(naija_pl_merge_24_25_perc_radar_top_3,
                            grid.min = 0,
                            grid.mid = 50,
                            grid.max = 100,
                            values.radar = c("", "", ""),
...
ggsave("sub_pro_6_naija_pl_analysis/images/24_25/naija_pl_analysis_24_25_perc_radar_top_3.png", height = 12, width = 12, dpi = 300)


# Bottom 3 Teams (Percent Metrics)

naija_pl_merge_24_25_perc_radar_bottom_3 <- naija_pl_merge_24_25_perc_radar |>
...
perc_radar_bottom_3 <- ggradar(naija_pl_merge_24_25_perc_radar_bottom_3,
                               grid.min = 0,
                               grid.mid = 50,
                               grid.max = 100,
                               values.radar = c("", "", ""),
...
ggsave("sub_pro_6_naija_pl_analysis/images/24_25/naija_pl_analysis_24_25_perc_radar_bottom_3.png", height = 12, width = 12, dpi = 300)


# Top Bottom 2 Teams (Percent Metrics)

naija_pl_merge_24_25_perc_radar_top_bottom_2 <- naija_pl_merge_24_25_perc_radar |>
...
perc_radar_top_bottom_2 <- ggradar(naija_pl_merge_24_25_perc_radar_top_bottom_2,
                                   grid.min = 0,
                                   grid.mid = 50,
                                   grid.max = 100,
                                   values.radar = c("", "", ""),
...
ggsave("sub_pro_6_naija_pl_analysis/images/24_25/naija_pl_analysis_24_25_perc_radar_top_bottom_2.png", height = 12, width = 12, dpi = 300)
