...
fifa_tm_top_player_value_clean_1 %>%
  ...
  geom_text_repel(aes(label = ifelse(ranking_aug_2023 < 50, name_country, "")), size = 8, 
                  min.segment.length = 0, seed = 42, box.padding = 0.5) +
  geom_text_repel(aes(label = ifelse(ranking_aug_2023 >= 95 & ranking_aug_2023 < 110, name_country, "")), size = 8,
                  min.segment.length = 0, seed = 42, box.padding = 0.5) +
  geom_text_repel(aes(label = ifelse(ranking_aug_2023 > 140, name_country, "")), size = 8,
                  min.segment.length = 0, seed = 42, box.padding = 0.5) +
...
fifa_tm_top_player_value_clean_1 %>%
  ggplot(aes(ranking_aug_2023, value_euros)) + 
  geom_point(aes(color= region), size = 4) +
  geom_text_repel(aes(label = ifelse(ranking_aug_2023 < 50, name_country, "")), size = 5,
                  min.segment.length = 0, seed = 42, box.padding = 0.5) +
  geom_text_repel(aes(label = ifelse(ranking_aug_2023 >= 95 & ranking_aug_2023 < 110, name_country, "")), size = 5, 
                  min.segment.length = 0, seed = 42, box.padding = 0.5) +
  geom_text_repel(aes(label = ifelse(ranking_aug_2023 > 140, name_country, "")), size = 5, 
                  min.segment.length = 0, seed = 42, box.padding = 0.5) +
  labs(x = "FIFA Ranking (Aug 2023)",
       y = "Market Value (Euros)",
       title = "FIFA ranking correlates with a team's\nhighest player value",
       subtitle = "Comparing FIFA ranking with the market value of\nthe top footballers in East and West Africa",
       caption = "Data Source: transfermarkt     |     By: @afro_dataviz",
       color = "Regions") +
  theme_classic() +
  scale_y_log10(labels  = 
...
fifa_tm_top_player_value_clean_1 %>%
  ...
  geom_text(aes(label = paste(name, country, sep = " - "), hjust = 0), size = 7) +
  coord_flip() +
  theme_minimal() +
  labs(x = "",
       y = "Market Value (Euros)",
  ...
  theme(axis.title.x =element_text(size = 18, vjust = -2),
        ...
        plot.subtitle = element_text(family="Helvetica", face="bold", size = 15, hjust = 0.5),
        plot.caption = element_text(family = "Helvetica",size = 12, face = "bold"),
        plot.background = element_rect(fill = "azure2", colour = "azure2"),
        panel.background = element_rect(fill = "azure2", colour = "azure2"),
        legend.title = element_text(size = 14),
...
fifa_tm_top_player_value_clean_1 %>%
  ...
  scale_y_continuous(limits = c(0, 150000000), breaks = seq(0, 150000000, 25000000), labels = comma) +
  scale_fill_manual(values = c("goldenrod2", "darkgreen"), labels = c('East Africa', 'West Africa')) +
  geom_text(aes(label = paste(name, country, sep = " - "), hjust = -0.05), size = 7) +
  coord_flip() +
  theme_minimal() +
  labs(x = "",
       y = "Market Value (Euros)",
...
ggsave("images/fifa_tm_top_player/market_value_portrait.png", width = 9, height = 16, dpi = 72)
