# Uganda PL League Table Analysis (2022-2023)
...
# page_22_23 <- read_html("https://www.espn.com/soccer/standings/_/league/UGA.1/season/2022")
# tables_22_23 <- html_table(page_22_23)
# uganda_pl_1_22_23 <- tables_22_23[[1]]
# uganda_pl_2_22_23 <- tables_22_23[[2]]
# 
# uganda_pl_1_22_23 <- uganda_pl_1_22_23 |>
...
#   select(-matches)
# 
# uganda_pl_merge_22_23 <- bind_cols(uganda_pl_1_22_23, uganda_pl_2_22_23) |>
#   mutate(team_name = replace(team_name, team_name == "AKCCA", "KCCA")) |>
#   mutate(team_name = replace(team_name, team_name == "FUPDF", "UPDF")) |>
#   mutate(team_name = replace(team_name, team_name == "lacks Power", "Blacks Power"))
# 
# uganda_pl_merge_22_23 <- uganda_pl_merge_22_23 |>
#   select(-Club)
# 
# # Save data as csv in datasets
# write_csv(uganda_pl_merge_22_23, here::here("sub_pro_3_uganda_pl_analysis",
#                                      "datasets", "uganda_pl_merge_22_23.csv"))


# Read in data
uganda_pl_merge_22_23 <- read_csv(here::here("sub_pro_3_uganda_pl_analysis", 
                                             "datasets", "uganda_pl_merge_22_23.csv"))
...
str(uganda_pl_merge_22_23)
head(uganda_pl_merge_22_23)

# 1) Plot of Points and Goal Differences in a Bar Chart

uganda_pl_merge_bar_22_23_pt_gd <- uganda_pl_merge_22_23 |>
  select(team_name, GD, P)

team_order <- uganda_pl_merge_bar_22_23_pt_gd %>%
  arrange(desc(P), desc(GD)) %>%
  pull(team_name)

uganda_pl_merge_bar_22_23_pt_gd_long <- uganda_pl_merge_bar_22_23_pt_gd |>
  pivot_longer(cols = c(P, GD), names_to = "metric", values_to = "value")

uganda_pl_merge_bar_22_23_pt_gd_long %>%
  mutate(team_name = factor(team_name, levels = rev(team_order))) %>%
  ggplot(aes(x = team_name, y = value, fill = metric)) +
  geom_col(position = position_dodge()) +
  geom_text(data = uganda_pl_merge_bar_22_23_pt_gd,
...
ggsave("sub_pro_3_uganda_pl_analysis/images/22_23/uganda_pl_analysis_22_23_pt_gd.png", width = 12, height = 12, dpi = 300)

# 2) Plot of Wins and Losses in a Lollipop Chart

uganda_pl_merge_lollipop_22_23_w_l <- uganda_pl_merge_22_23 |>
...
ggplot(uganda_pl_merge_lollipop_22_23_w_l) +
...
ggsave("sub_pro_3_uganda_pl_analysis/images/22_23/uganda_pl_analysis_22_23_w_l.png", width = 12, height = 12, dpi = 300)

# 3) Plot of Points per Goal

uganda_pl_merge_bar_22_23_ppg <- uganda_pl_merge_22_23 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
  select(team_name, PPG) 

ggplot(uganda_pl_merge_bar_22_23_ppg) +
...
ggsave("sub_pro_3_uganda_pl_analysis/images/22_23/uganda_pl_analysis_22_23_ppg.png", width = 12, height = 12, dpi = 300)

# 4) Plot of Goals per Game

uganda_pl_merge_bar_22_23_gpg <- uganda_pl_merge_22_23 |>
  mutate(GPG = round(F/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GPG) 

ggplot(uganda_pl_merge_bar_22_23_gpg) +
...
ggsave("sub_pro_3_uganda_pl_analysis/images/22_23/uganda_pl_analysis_22_23_gpg.png", width = 12, height = 12, dpi = 300)


# 5) Plot of Goals Against per Game

uganda_pl_merge_bar_22_23_gapg <- uganda_pl_merge_22_23 |>
  mutate(GAPG = round(A/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GAPG) 

ggplot(uganda_pl_merge_bar_22_23_gapg) +
...
ggsave("sub_pro_3_uganda_pl_analysis/images/22_23/uganda_pl_analysis_22_23_ga_pg.png", width = 12, height = 12, dpi = 300)


# 6) Plot of Goal Difference per Game

uganda_pl_merge_bar_22_23_gdpg <- uganda_pl_merge_22_23 |>
  mutate(GDPG = round(GD/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GDPG) 

ggplot(uganda_pl_merge_bar_22_23_gdpg) +
...
ggsave("sub_pro_3_uganda_pl_analysis/images/22_23/uganda_pl_analysis_22_23_gd_pg.png", width = 12, height = 12, dpi = 300)

# 7) Scatterplot of GF vs GA

uganda_pl_merge_bar_22_23_gf_ga <- uganda_pl_merge_22_23 |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
  select(team_name, F, A)

# Thresholds
against_thresh <- median(uganda_pl_merge_bar_22_23_gf_ga$A)
for_thresh <- median(uganda_pl_merge_bar_22_23_gf_ga$F)

ggplot(uganda_pl_merge_bar_22_23_gf_ga, aes(x = A, y = F)) +
...
ggsave("sub_pro_3_uganda_pl_analysis/images/22_23/uganda_pl_analysis_22_23_gf_ga.png", width = 12, height = 12, dpi = 300)


# 8) Scatterplot of PPG vs GD

uganda_pl_merge_bar_22_23_ppg_gd <- uganda_pl_merge_22_23 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
...
# Thresholds
goal_diff_thresh <- median(uganda_pl_merge_bar_22_23_ppg_gd$GD)
ppg_thresh <- median(uganda_pl_merge_bar_22_23_ppg_gd$PPG)

ggplot(uganda_pl_merge_bar_22_23_ppg_gd, aes(x = GD, y = PPG)) +
  geom_point(color = "brown4", size = 6) +
  geom_text_repel(aes(label = team_name), vjust = -0.5, size = 8) +
...
ggsave("sub_pro_3_uganda_pl_analysis/images/22_23/uganda_pl_analysis_22_23_ppg_gd.png", width = 12, height = 12, dpi = 300)
...
uganda_pl_merge_22_23_non_perc <- uganda_pl_merge_22_23 |>
  mutate(
    PPG = P / GP,
    GPG = F / GP,
    GAPG = A / GP,
...
uganda_pl_merge_22_23_perc <- uganda_pl_merge_22_23 |>
  mutate(
    WinPerc = (W / GP) * 100,
    DrawPerc = (D / GP) * 100,
    LossPerc = (L / GP) * 100,
...
uganda_pl_merge_22_23_non_perc_radar <- uganda_pl_merge_22_23_non_perc |>
  select(team_name, PPG, GPG, GAPG, GF2GA, GDPG, GD2P) 

uganda_pl_merge_22_23_perc_radar <- uganda_pl_merge_22_23_perc |>
...
ggsave("sub_pro_3_uganda_pl_analysis/images/22_23/uganda_pl_analysis_22_23_perc_radar_top_3.png", height = 12, width = 12, dpi = 300)
...
ggsave("sub_pro_3_uganda_pl_analysis/images/22_23/uganda_pl_analysis_22_23_perc_radar_bottom_3.png", height = 12, width = 12, dpi = 300)
...
ggsave("sub_pro_3_uganda_pl_analysis/images/22_23/uganda_pl_analysis_22_23_perc_radar_top_bottom_2.png", height = 12, width = 12, dpi = 300)
