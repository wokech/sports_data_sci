# Zambia PL League Table Analysis (2021-2022)
...
# page_21_22 <- read_html("https://www.espn.com/soccer/standings/_/league/ZAM.1/season/2021")
# tables_21_22 <- html_table(page_21_22)
# zam_pl_1_21_22 <- tables_21_22[[1]]
# zam_pl_2_21_22 <- tables_21_22[[2]]
# 
# zam_pl_1_21_22 <- zam_pl_1_21_22 |>
#   rename(Club = `2021-22 ZSL`)
# 
# zam_pl_1_21_22 <- zam_pl_1_21_22 |>
#   mutate(
...
#   select(-matches)
# 
# zam_pl_merge_21_22 <- bind_cols(zam_pl_1_21_22, zam_pl_2_21_22)
# 
# zam_pl_merge_21_22 <- zam_pl_merge_21_22 |>
#   select(-Club)
# 
# # Save data as csv in datasets
# write_csv(zam_pl_merge_21_22, here::here("sub_pro_5_zam_pl_analysis",
#                                          "datasets", "zam_pl_merge_21_22.csv"))

# Read in data
zam_pl_merge_21_22 <- read_csv(here::here("sub_pro_5_zam_pl_analysis", 
                                          "datasets", "zam_pl_merge_21_22.csv"))
...
str(zam_pl_merge_21_22)
head(zam_pl_merge_21_22)

# 1) Plot of Points and Goal Differences in a Bar Chart

zam_pl_merge_bar_21_22_pt_gd <- zam_pl_merge_21_22 |>
  select(team_name, GD, P)

team_order <- zam_pl_merge_bar_21_22_pt_gd %>%
  arrange(desc(P), desc(GD)) %>%
  pull(team_name)

zam_pl_merge_bar_21_22_pt_gd_long <- zam_pl_merge_bar_21_22_pt_gd |>
  pivot_longer(cols = c(P, GD), names_to = "metric", values_to = "value")

# Plot

zam_pl_merge_bar_21_22_pt_gd_long %>%
  mutate(team_name = factor(team_name, levels = rev(team_order))) %>%
  ggplot(aes(x = team_name, y = value, fill = metric)) +
  geom_col(position = position_dodge()) +
  geom_text(data = zam_pl_merge_bar_21_22_pt_gd,
...
ggsave("sub_pro_5_zam_pl_analysis/images/21_22/zam_pl_analysis_21_22_pt_gd.png", width = 12, height = 12, dpi = 300)

# 2) Plot of Wins and Losses in a Lollipop Chart

zam_pl_merge_lollipop_21_22_w_l <- zam_pl_merge_21_22 |>
...
ggplot(zam_pl_merge_lollipop_21_22_w_l) +
...
ggsave("sub_pro_5_zam_pl_analysis/images/21_22/zam_pl_analysis_21_22_w_l.png", width = 12, height = 12, dpi = 300)

# 3) Plot of Points per Goal

zam_pl_merge_bar_21_22_ppg <- zam_pl_merge_21_22 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
  select(team_name, PPG) 

ggplot(zam_pl_merge_bar_21_22_ppg) +
...
ggsave("sub_pro_5_zam_pl_analysis/images/21_22/zam_pl_analysis_21_22_ppg.png", width = 12, height = 12, dpi = 300)

# 4) Plot of Goals per Game

zam_pl_merge_bar_21_22_gpg <- zam_pl_merge_21_22 |>
  mutate(GPG = round(F/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GPG) 

ggplot(zam_pl_merge_bar_21_22_gpg) +
...
ggsave("sub_pro_5_zam_pl_analysis/images/21_22/zam_pl_analysis_21_22_gpg.png", width = 12, height = 12, dpi = 300)


# 5) Plot of Goals Against per Game

zam_pl_merge_bar_21_22_gapg <- zam_pl_merge_21_22 |>
  mutate(GAPG = round(A/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GAPG) 

ggplot(zam_pl_merge_bar_21_22_gapg) +
...
ggsave("sub_pro_5_zam_pl_analysis/images/21_22/zam_pl_analysis_21_22_ga_pg.png", width = 12, height = 12, dpi = 300)


# 6) Plot of Goal Difference per Game

zam_pl_merge_bar_21_22_gdpg <- zam_pl_merge_21_22 |>
  mutate(GDPG = round(GD/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GDPG) 

ggplot(zam_pl_merge_bar_21_22_gdpg) +
...
ggsave("sub_pro_5_zam_pl_analysis/images/21_22/zam_pl_analysis_21_22_gd_pg.png", width = 12, height = 12, dpi = 300)

# 7) Scatterplot of GF vs GA

zam_pl_merge_bar_21_22_gf_ga <- zam_pl_merge_21_22 |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
  select(team_name, F, A)

# Thresholds
against_thresh <- median(zam_pl_merge_bar_21_22_gf_ga$A)
for_thresh <- median(zam_pl_merge_bar_21_22_gf_ga$F)

ggplot(zam_pl_merge_bar_21_22_gf_ga, aes(x = A, y = F)) +
...
ggsave("sub_pro_5_zam_pl_analysis/images/21_22/zam_pl_analysis_21_22_gf_ga.png", width = 12, height = 12, dpi = 300)


# 8) Scatterplot of PPG vs GD

zam_pl_merge_bar_21_22_ppg_gd <- zam_pl_merge_21_22 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
...
# Thresholds
goal_diff_thresh <- median(zam_pl_merge_bar_21_22_ppg_gd$GD)
ppg_thresh <- median(zam_pl_merge_bar_21_22_ppg_gd$PPG)

ggplot(zam_pl_merge_bar_21_22_ppg_gd, aes(x = GD, y = PPG)) +
  geom_point(color = "brown4", size = 6) +
  geom_text_repel(aes(label = team_name), vjust = -0.5, size = 8) +
...
ggsave("sub_pro_5_zam_pl_analysis/images/21_22/zam_pl_analysis_21_22_ppg_gd.png", width = 12, height = 12, dpi = 300)
...
zam_pl_merge_21_22_non_perc <- zam_pl_merge_21_22 |>
  mutate(
    PPG = P / GP,
    GPG = F / GP,
    GAPG = A / GP,
...
zam_pl_merge_21_22_perc <- zam_pl_merge_21_22 |>
  mutate(
    WinPerc = (W / GP) * 100,
    DrawPerc = (D / GP) * 100,
    LossPerc = (L / GP) * 100,
...
zam_pl_merge_21_22_non_perc_radar <- zam_pl_merge_21_22_non_perc |>
  select(team_name, PPG, GPG, GAPG, GF2GA, GDPG, GD2P) 

zam_pl_merge_21_22_perc_radar <- zam_pl_merge_21_22_perc |>
...
zam_pl_merge_21_22_perc_radar_top_3 <- zam_pl_merge_21_22_perc_radar |>
  filter(team_name == c("Red Arrows", "Zesco United", "Green Eagles"))

my_top_colors <- c("Red Arrows" = "#BE8125", 
                   "Zesco United" = "#2FBE25", 
                   "Green Eagles" = "#BE25AB")

perc_radar_top_3 <- ggradar(zam_pl_merge_21_22_perc_radar_top_3,
                            grid.min = 0,
                            grid.mid = 50,
                            grid.max = 100,
                            values.radar = c("", "", ""),
...
ggsave("sub_pro_5_zam_pl_analysis/images/21_22/zam_pl_analysis_21_22_perc_radar_top_3.png", height = 12, width = 12, dpi = 300)


# Bottom 3 Teams (Percent Metrics)

zam_pl_merge_21_22_perc_radar_bottom_3 <- zam_pl_merge_21_22_perc_radar |>
  filter(team_name == c("Kafue Celtic", "Indeni", "Konkola Blades"))

my_bottom_colors <- c("Kafue Celtic" = "#BE8125", 
                      "Indeni" = "#2FBE25", 
                      "Konkola Blades" = "#BE25AB")

perc_radar_bottom_3 <- ggradar(zam_pl_merge_21_22_perc_radar_bottom_3,
                               grid.min = 0,
                               grid.mid = 50,
                               grid.max = 100,
                               values.radar = c("", "", ""),
...
ggsave("sub_pro_5_zam_pl_analysis/images/21_22/zam_pl_analysis_21_22_perc_radar_bottom_3.png", height = 12, width = 12, dpi = 300)


# Top Bottom 2 Teams (Percent Metrics)

zam_pl_merge_21_22_perc_radar_top_bottom_2 <- zam_pl_merge_21_22_perc_radar |>
  filter(team_name %in% c("Red Arrows", "Zesco United", "Indeni", "Konkola Blades"))
...
perc_radar_top_bottom_2 <- ggradar(zam_pl_merge_21_22_perc_radar_top_bottom_2,
                                   grid.min = 0,
                                   grid.mid = 50,
                                   grid.max = 100,
                                   values.radar = c("", "", ""),
...
ggsave("sub_pro_5_zam_pl_analysis/images/21_22/zam_pl_analysis_21_22_perc_radar_top_bottom_2.png", height = 12, width = 12, dpi = 300)
