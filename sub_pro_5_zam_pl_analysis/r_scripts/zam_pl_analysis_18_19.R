# Zambia PL League Table Analysis (2018-2019)
...
# page_18_19 <- read_html("https://www.espn.com/soccer/standings/_/league/ZAM.1/season/2018")
# tables_18_19 <- html_table(page_18_19)
# zam_pl_1_18_19 <- tables_18_19[[1]]
# zam_pl_2_18_19 <- tables_18_19[[2]]
# 
# zam_pl_1_18_19 <- zam_pl_1_18_19 |>
#   rename(Club = `Super League`)
# 
# zam_pl_1_18_19 <- zam_pl_1_18_19 |>
#   mutate(
...
#   select(-matches)
# 
# zam_pl_merge_18_19 <- bind_cols(zam_pl_1_18_19, zam_pl_2_18_19)
# 
# zam_pl_merge_18_19 <- zam_pl_merge_18_19 |>
#   select(-Club)
# 
# # Save data as csv in datasets
# write_csv(zam_pl_merge_18_19, here::here("sub_pro_5_zam_pl_analysis",
#                                          "datasets", "zam_pl_merge_18_19.csv"))

# Read in data
zam_pl_merge_18_19 <- read_csv(here::here("sub_pro_5_zam_pl_analysis", 
                                          "datasets", "zam_pl_merge_18_19.csv"))
...
str(zam_pl_merge_18_19)
head(zam_pl_merge_18_19)

# 1) Plot of Points and Goal Differences in a Bar Chart

zam_pl_merge_bar_18_19_pt_gd <- zam_pl_merge_18_19 |>
  select(team_name, GD, P)

team_order <- zam_pl_merge_bar_18_19_pt_gd %>%
  arrange(desc(P), desc(GD)) %>%
  pull(team_name)

zam_pl_merge_bar_18_19_pt_gd_long <- zam_pl_merge_bar_18_19_pt_gd |>
  pivot_longer(cols = c(P, GD), names_to = "metric", values_to = "value")

# Plot

zam_pl_merge_bar_18_19_pt_gd_long %>%
  mutate(team_name = factor(team_name, levels = rev(team_order))) %>%
  ggplot(aes(x = team_name, y = value, fill = metric)) +
  geom_col(position = position_dodge()) +
  geom_text(data = zam_pl_merge_bar_18_19_pt_gd,
...
ggsave("sub_pro_5_zam_pl_analysis/images/18_19/zam_pl_analysis_18_19_pt_gd.png", width = 12, height = 12, dpi = 300)

# 2) Plot of Wins and Losses in a Lollipop Chart

zam_pl_merge_lollipop_18_19_w_l <- zam_pl_merge_18_19 |>
...
ggplot(zam_pl_merge_lollipop_18_19_w_l) +
...
ggsave("sub_pro_5_zam_pl_analysis/images/18_19/zam_pl_analysis_18_19_w_l.png", width = 12, height = 12, dpi = 300)

# 3) Plot of Points per Goal

zam_pl_merge_bar_18_19_ppg <- zam_pl_merge_18_19 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
  select(team_name, PPG) 

ggplot(zam_pl_merge_bar_18_19_ppg) +
...
ggsave("sub_pro_5_zam_pl_analysis/images/18_19/zam_pl_analysis_18_19_ppg.png", width = 12, height = 12, dpi = 300)

# 4) Plot of Goals per Game

zam_pl_merge_bar_18_19_gpg <- zam_pl_merge_18_19 |>
  mutate(GPG = round(F/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GPG) 

ggplot(zam_pl_merge_bar_18_19_gpg) +
...
ggsave("sub_pro_5_zam_pl_analysis/images/18_19/zam_pl_analysis_18_19_gpg.png", width = 12, height = 12, dpi = 300)


# 5) Plot of Goals Against per Game

zam_pl_merge_bar_18_19_gapg <- zam_pl_merge_18_19 |>
  mutate(GAPG = round(A/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GAPG) 

ggplot(zam_pl_merge_bar_18_19_gapg) +
...
ggsave("sub_pro_5_zam_pl_analysis/images/18_19/zam_pl_analysis_18_19_ga_pg.png", width = 12, height = 12, dpi = 300)


# 6) Plot of Goal Difference per Game

zam_pl_merge_bar_18_19_gdpg <- zam_pl_merge_18_19 |>
  mutate(GDPG = round(GD/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GDPG) 

ggplot(zam_pl_merge_bar_18_19_gdpg) +
...
ggsave("sub_pro_5_zam_pl_analysis/images/18_19/zam_pl_analysis_18_19_gd_pg.png", width = 12, height = 12, dpi = 300)

# 7) Scatterplot of GF vs GA

zam_pl_merge_bar_18_19_gf_ga <- zam_pl_merge_18_19 |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
  select(team_name, F, A)

# Thresholds
against_thresh <- median(zam_pl_merge_bar_18_19_gf_ga$A)
for_thresh <- median(zam_pl_merge_bar_18_19_gf_ga$F)

ggplot(zam_pl_merge_bar_18_19_gf_ga, aes(x = A, y = F)) +
...
ggsave("sub_pro_5_zam_pl_analysis/images/18_19/zam_pl_analysis_18_19_gf_ga.png", width = 12, height = 12, dpi = 300)


# 8) Scatterplot of PPG vs GD

zam_pl_merge_bar_18_19_ppg_gd <- zam_pl_merge_18_19 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
...
# Thresholds
goal_diff_thresh <- median(zam_pl_merge_bar_18_19_ppg_gd$GD)
ppg_thresh <- median(zam_pl_merge_bar_18_19_ppg_gd$PPG)

ggplot(zam_pl_merge_bar_18_19_ppg_gd, aes(x = GD, y = PPG)) +
  geom_point(color = "brown4", size = 6) +
  geom_text_repel(aes(label = team_name), vjust = -0.5, size = 8) +
...
ggsave("sub_pro_5_zam_pl_analysis/images/18_19/zam_pl_analysis_18_19_ppg_gd.png", width = 12, height = 12, dpi = 300)
...
zam_pl_merge_18_19_non_perc <- zam_pl_merge_18_19 |>
  mutate(
    PPG = P / GP,
    GPG = F / GP,
    GAPG = A / GP,
...
zam_pl_merge_18_19_perc <- zam_pl_merge_18_19 |>
  mutate(
    WinPerc = (W / GP) * 100,
    DrawPerc = (D / GP) * 100,
    LossPerc = (L / GP) * 100,
...
zam_pl_merge_18_19_non_perc_radar <- zam_pl_merge_18_19_non_perc |>
  select(team_name, PPG, GPG, GAPG, GF2GA, GDPG, GD2P) 

zam_pl_merge_18_19_perc_radar <- zam_pl_merge_18_19_perc |>
...
zam_pl_merge_18_19_perc_radar_top_3 <- zam_pl_merge_18_19_perc_radar |>
  filter(team_name == c("Zesco United", "Nkana", "Green Buffaloes"))

my_top_colors <- c("Zesco United" = "#BE8125", 
                   "Nkana" = "#2FBE25", 
                   "Green Buffaloes" = "#BE25AB")

perc_radar_top_3 <- ggradar(zam_pl_merge_18_19_perc_radar_top_3,
                            grid.min = 0,
                            grid.mid = 50,
                            grid.max = 100,
                            values.radar = c("", "", ""),
...
ggsave("sub_pro_5_zam_pl_analysis/images/18_19/zam_pl_analysis_18_19_perc_radar_top_3.png", height = 12, width = 12, dpi = 300)


# Bottom 3 Teams (Percent Metrics)

zam_pl_merge_18_19_perc_radar_bottom_3 <- zam_pl_merge_18_19_perc_radar |>
  filter(team_name %in% c("Kabwe YSA", "Nchanga Rangers", "New Monze Swallows"))

my_bottom_colors <- c("Kabwe YSA" = "#BE8125", 
                      "Nchanga Rangers" = "#2FBE25", 
                      "New Monze Swallows" = "#BE25AB")

perc_radar_bottom_3 <- ggradar(zam_pl_merge_18_19_perc_radar_bottom_3,
                               grid.min = 0,
                               grid.mid = 50,
                               grid.max = 100,
                               values.radar = c("", "", ""),
...
ggsave("sub_pro_5_zam_pl_analysis/images/18_19/zam_pl_analysis_18_19_perc_radar_bottom_3.png", height = 12, width = 12, dpi = 300)


# Top Bottom 2 Teams (Percent Metrics)

zam_pl_merge_18_19_perc_radar_top_bottom_2 <- zam_pl_merge_18_19_perc_radar |>
  filter(team_name %in% c("Zesco United", "Nkana", "Nchanga Rangers", "New Monze Swallows"))

my_top_bottom_colors <- c("Zesco United" = "#000080", 
                          "Nkana" = "#2FBE25", 
                          "Nchanga Rangers" = "#BE8125", 
                          "New Monze Swallows" = "#BE25AB")

perc_radar_top_bottom_2 <- ggradar(zam_pl_merge_18_19_perc_radar_top_bottom_2,
                                   grid.min = 0,
                                   grid.mid = 50,
                                   grid.max = 100,
                                   values.radar = c("", "", ""),
...
ggsave("sub_pro_5_zam_pl_analysis/images/18_19/zam_pl_analysis_18_19_perc_radar_top_bottom_2.png", height = 12, width = 12, dpi = 300)
