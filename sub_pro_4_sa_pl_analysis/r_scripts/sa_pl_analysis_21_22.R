# South Africa PL League Table Analysis (2021-2022)
...
# Read in data
sa_pl_merge_21_22 <- read_csv(here::here("sub_pro_4_sa_pl_analysis", 
                                       "datasets", "sa_pl_merge_21_22.csv"))
...
str(sa_pl_merge_21_22)
head(sa_pl_merge_21_22)

# 1) Plot of Points and Goal Differences in a Bar Chart

sa_pl_merge_bar_21_22_pt_gd <- sa_pl_merge_21_22 |>
  select(team_name, GD, P)

team_order <- sa_pl_merge_bar_21_22_pt_gd %>%
  arrange(desc(P), desc(GD)) %>%
  pull(team_name)

sa_pl_merge_bar_21_22_pt_gd_long <- sa_pl_merge_bar_21_22_pt_gd |>
  pivot_longer(cols = c(P, GD), names_to = "metric", values_to = "value")

# Plot

sa_pl_merge_bar_21_22_pt_gd_long %>%
  mutate(team_name = factor(team_name, levels = rev(team_order))) %>%
  ggplot(aes(x = team_name, y = value, fill = metric)) +
  geom_col(position = position_dodge()) +
  geom_text(data = sa_pl_merge_bar_21_22_pt_gd,
...
ggsave("sub_pro_4_sa_pl_analysis/images/21_22/sa_pl_analysis_21_22_pt_gd.png", width = 12, height = 12, dpi = 300)

# 2) Plot of Wins and Losses in a Lollipop Chart

sa_pl_merge_lollipop_21_22_w_l <- sa_pl_merge_21_22 |>
...
ggplot(sa_pl_merge_lollipop_21_22_w_l) +
...
ggsave("sub_pro_4_sa_pl_analysis/images/21_22/sa_pl_analysis_21_22_w_l.png", width = 12, height = 12, dpi = 300)

# 3) Plot of Points per Goal

sa_pl_merge_bar_21_22_ppg <- sa_pl_merge_21_22 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
  select(team_name, PPG) 

ggplot(sa_pl_merge_bar_21_22_ppg) +
...
ggsave("sub_pro_4_sa_pl_analysis/images/21_22/sa_pl_analysis_21_22_ppg.png", width = 12, height = 12, dpi = 300)

# 4) Plot of Goals per Game

sa_pl_merge_bar_21_22_gpg <- sa_pl_merge_21_22 |>
  mutate(GPG = round(F/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GPG) 

ggplot(sa_pl_merge_bar_21_22_gpg) +
...
ggsave("sub_pro_4_sa_pl_analysis/images/21_22/sa_pl_analysis_21_22_gpg.png", width = 12, height = 12, dpi = 300)


# 5) Plot of Goals Against per Game

sa_pl_merge_bar_21_22_gapg <- sa_pl_merge_21_22 |>
  mutate(GAPG = round(A/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GAPG) 

ggplot(sa_pl_merge_bar_21_22_gapg) +
...
ggsave("sub_pro_4_sa_pl_analysis/images/21_22/sa_pl_analysis_21_22_ga_pg.png", width = 12, height = 12, dpi = 300)


# 6) Plot of Goal Difference per Game

sa_pl_merge_bar_21_22_gdpg <- sa_pl_merge_21_22 |>
  mutate(GDPG = round(GD/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GDPG) 

ggplot(sa_pl_merge_bar_21_22_gdpg) +
...
ggsave("sub_pro_4_sa_pl_analysis/images/21_22/sa_pl_analysis_21_22_gd_pg.png", width = 12, height = 12, dpi = 300)

# 7) Scatterplot of GF vs GA

sa_pl_merge_bar_21_22_gf_ga <- sa_pl_merge_21_22 |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
  select(team_name, F, A)

# Thresholds
against_thresh <- median(sa_pl_merge_bar_21_22_gf_ga$A)
for_thresh <- median(sa_pl_merge_bar_21_22_gf_ga$F)

ggplot(sa_pl_merge_bar_21_22_gf_ga, aes(x = A, y = F)) +
...
ggsave("sub_pro_4_sa_pl_analysis/images/21_22/sa_pl_analysis_21_22_gf_ga.png", width = 12, height = 12, dpi = 300)


# 8) Scatterplot of PPG vs GD

sa_pl_merge_bar_21_22_ppg_gd <- sa_pl_merge_21_22 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
...
# Thresholds
goal_diff_thresh <- median(sa_pl_merge_bar_21_22_ppg_gd$GD)
ppg_thresh <- median(sa_pl_merge_bar_21_22_ppg_gd$PPG)

ggplot(sa_pl_merge_bar_21_22_ppg_gd, aes(x = GD, y = PPG)) +
  geom_point(color = "brown4", size = 6) +
  geom_text_repel(aes(label = team_name), vjust = -0.5, size = 8) +
...
ggsave("sub_pro_4_sa_pl_analysis/images/21_22/sa_pl_analysis_21_22_ppg_gd.png", width = 12, height = 12, dpi = 300)
...
sa_pl_merge_21_22_non_perc <- sa_pl_merge_21_22 |>
  mutate(
    PPG = P / GP,
    GPG = F / GP,
    GAPG = A / GP,
...
sa_pl_merge_21_22_perc <- sa_pl_merge_21_22 |>
  mutate(
    WinPerc = (W / GP) * 100,
    DrawPerc = (D / GP) * 100,
    LossPerc = (L / GP) * 100,
...
sa_pl_merge_21_22_non_perc_radar <- sa_pl_merge_21_22_non_perc |>
  select(team_name, PPG, GPG, GAPG, GF2GA, GDPG, GD2P) 

sa_pl_merge_21_22_perc_radar <- sa_pl_merge_21_22_perc |>
...
ggsave("sub_pro_4_sa_pl_analysis/images/21_22/sa_pl_analysis_21_22_perc_radar_top_3.png", height = 12, width = 12, dpi = 300)
...
ggsave("sub_pro_4_sa_pl_analysis/images/21_22/sa_pl_analysis_21_22_perc_radar_bottom_3.png", height = 12, width = 12, dpi = 300)
...
ggsave("sub_pro_4_sa_pl_analysis/images/21_22/sa_pl_analysis_21_22_perc_radar_top_bottom_2.png", height = 12, width = 12, dpi = 300)
