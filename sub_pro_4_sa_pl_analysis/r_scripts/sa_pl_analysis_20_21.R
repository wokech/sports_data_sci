# South Africa PL League Table Analysis (202-2021)
...
# page_20_21 <- read_html("https://www.espn.com/soccer/standings/_/league/RSA.1/seasontype/1/season/2020")
# tables_20_21 <- html_table(page_20_21)
# sa_pl_1_20_21 <- tables_20_21[[1]]
# sa_pl_2_20_21 <- tables_20_21[[2]]
# 
# sa_pl_1_20_21 <- sa_pl_1_20_21 |>
#   rename(Club = `2020-21`)
# 
# sa_pl_1_20_21 <- sa_pl_1_20_21 |>
#   mutate(
...
#   select(-matches)
# 
# sa_pl_merge_20_21 <- bind_cols(sa_pl_1_20_21, sa_pl_2_20_21) |>
...
# 
# sa_pl_merge_20_21 <- sa_pl_merge_20_21 |>
#   select(-Club)
# 
# # Save data as csv in datasets
# write_csv(sa_pl_merge_20_21, here::here("sub_pro_4_sa_pl_analysis",
#                                      "datasets", "sa_pl_merge_20_21.csv"))


# Read in data
sa_pl_merge_20_21 <- read_csv(here::here("sub_pro_4_sa_pl_analysis", 
                                       "datasets", "sa_pl_merge_20_21.csv"))
...
str(sa_pl_merge_20_21)
head(sa_pl_merge_20_21)

# 1) Plot of Points and Goal Differences in a Bar Chart

sa_pl_merge_bar_20_21_pt_gd <- sa_pl_merge_20_21 |>
  select(team_name, GD, P)

team_order <- sa_pl_merge_bar_20_21_pt_gd %>%
  arrange(desc(P), desc(GD)) %>%
  pull(team_name)

sa_pl_merge_bar_20_21_pt_gd_long <- sa_pl_merge_bar_20_21_pt_gd |>
  pivot_longer(cols = c(P, GD), names_to = "metric", values_to = "value")

# Plot

sa_pl_merge_bar_20_21_pt_gd_long %>%
  mutate(team_name = factor(team_name, levels = rev(team_order))) %>%
  ggplot(aes(x = team_name, y = value, fill = metric)) +
  geom_col(position = position_dodge()) +
  geom_text(data = sa_pl_merge_bar_20_21_pt_gd,
...
ggsave("sub_pro_4_sa_pl_analysis/images/20_21/sa_pl_analysis_20_21_pt_gd.png", width = 12, height = 12, dpi = 300)

# 2) Plot of Wins and Losses in a Lollipop Chart

sa_pl_merge_lollipop_20_21_w_l <- sa_pl_merge_20_21 |>
...
ggplot(sa_pl_merge_lollipop_20_21_w_l) +
...
ggsave("sub_pro_4_sa_pl_analysis/images/20_21/sa_pl_analysis_20_21_w_l.png", width = 12, height = 12, dpi = 300)

# 3) Plot of Points per Goal

sa_pl_merge_bar_20_21_ppg <- sa_pl_merge_20_21 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
  select(team_name, PPG) 

ggplot(sa_pl_merge_bar_20_21_ppg) +
...
ggsave("sub_pro_4_sa_pl_analysis/images/20_21/sa_pl_analysis_20_21_ppg.png", width = 12, height = 12, dpi = 300)

# 4) Plot of Goals per Game

sa_pl_merge_bar_20_21_gpg <- sa_pl_merge_20_21 |>
  mutate(GPG = round(F/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GPG) 

ggplot(sa_pl_merge_bar_20_21_gpg) +
...
ggsave("sub_pro_4_sa_pl_analysis/images/20_21/sa_pl_analysis_20_21_gpg.png", width = 12, height = 12, dpi = 300)


# 5) Plot of Goals Against per Game

sa_pl_merge_bar_20_21_gapg <- sa_pl_merge_20_21 |>
  mutate(GAPG = round(A/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GAPG) 

ggplot(sa_pl_merge_bar_20_21_gapg) +
...
ggsave("sub_pro_4_sa_pl_analysis/images/20_21/sa_pl_analysis_20_21_ga_pg.png", width = 12, height = 12, dpi = 300)


# 6) Plot of Goal Difference per Game

sa_pl_merge_bar_20_21_gdpg <- sa_pl_merge_20_21 |>
  mutate(GDPG = round(GD/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GDPG) 

ggplot(sa_pl_merge_bar_20_21_gdpg) +
...
ggsave("sub_pro_4_sa_pl_analysis/images/20_21/sa_pl_analysis_20_21_gd_pg.png", width = 12, height = 12, dpi = 300)

# 7) Scatterplot of GF vs GA

sa_pl_merge_bar_20_21_gf_ga <- sa_pl_merge_20_21 |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
  select(team_name, F, A)

# Thresholds
against_thresh <- median(sa_pl_merge_bar_20_21_gf_ga$A)
for_thresh <- median(sa_pl_merge_bar_20_21_gf_ga$F)

ggplot(sa_pl_merge_bar_20_21_gf_ga, aes(x = A, y = F)) +
...
ggsave("sub_pro_4_sa_pl_analysis/images/20_21/sa_pl_analysis_20_21_gf_ga.png", width = 12, height = 12, dpi = 300)


# 8) Scatterplot of PPG vs GD

sa_pl_merge_bar_20_21_ppg_gd <- sa_pl_merge_20_21 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
...
# Thresholds
goal_diff_thresh <- median(sa_pl_merge_bar_20_21_ppg_gd$GD)
ppg_thresh <- median(sa_pl_merge_bar_20_21_ppg_gd$PPG)

ggplot(sa_pl_merge_bar_20_21_ppg_gd, aes(x = GD, y = PPG)) +
  geom_point(color = "brown4", size = 6) +
  geom_text_repel(aes(label = team_name), vjust = -0.5, size = 8) +
...
ggsave("sub_pro_4_sa_pl_analysis/images/20_21/sa_pl_analysis_20_21_ppg_gd.png", width = 12, height = 12, dpi = 300)
...
sa_pl_merge_20_21_non_perc <- sa_pl_merge_20_21 |>
  mutate(
    PPG = P / GP,
    GPG = F / GP,
    GAPG = A / GP,
...
sa_pl_merge_20_21_perc <- sa_pl_merge_20_21 |>
  mutate(
    WinPerc = (W / GP) * 100,
    DrawPerc = (D / GP) * 100,
    LossPerc = (L / GP) * 100,
...
sa_pl_merge_20_21_non_perc_radar <- sa_pl_merge_20_21_non_perc |>
  select(team_name, PPG, GPG, GAPG, GF2GA, GDPG, GD2P) 

sa_pl_merge_20_21_perc_radar <- sa_pl_merge_20_21_perc |>
...
sa_pl_merge_20_21_perc_radar_top_3 <- sa_pl_merge_20_21_perc_radar |>
  filter(team_name == c("Mamelodi Sundowns", "Amazulu", "Orlando Pirates"))

my_top_colors <- c("Mamelodi Sundowns" = "#BE8125", 
                   "Amazulu" = "#2FBE25", 
                   "Orlando Pirates" = "#BE25AB")

perc_radar_top_3 <- ggradar(sa_pl_merge_20_21_perc_radar_top_3,
                            grid.min = 0,
                            grid.mid = 50,
                            grid.max = 100,
                            values.radar = c("", "", ""),
...
ggsave("sub_pro_4_sa_pl_analysis/images/20_21/sa_pl_analysis_20_21_perc_radar_top_3.png", height = 12, width = 12, dpi = 300)


# Bottom 3 Teams (Percent Metrics)

sa_pl_merge_20_21_perc_radar_bottom_3 <- sa_pl_merge_20_21_perc_radar |>
  filter(team_name %in% c("Stellenbosch", "Chippa United", "Black Leopards"))

my_bottom_colors <- c("Stellenbosch" = "#BE8125", 
                      "Chippa United" = "#2FBE25", 
                      "Black Leopards" = "#BE25AB")

perc_radar_bottom_3 <- ggradar(sa_pl_merge_20_21_perc_radar_bottom_3,
                               grid.min = 0,
                               grid.mid = 50,
                               grid.max = 100,
                               values.radar = c("", "", ""),
...
ggsave("sub_pro_4_sa_pl_analysis/images/20_21/sa_pl_analysis_20_21_perc_radar_bottom_3.png", height = 12, width = 12, dpi = 300)


# Bottom 3 Teams (Percent Metrics)

sa_pl_merge_20_21_perc_radar_top_bottom_2 <- sa_pl_merge_20_21_perc_radar |>
  filter(team_name %in% c("Mamelodi Sundowns", "Amazulu", "Chippa United", "Black Leopards"))

my_top_bottom_colors <- c("Mamelodi Sundowns" = "#000080", 
                          "Amazulu" = "#2FBE25", 
                          "Chippa United" = "#BE8125", 
                          "Black Leopards" = "#BE25AB")

perc_radar_top_bottom_2 <- ggradar(sa_pl_merge_20_21_perc_radar_top_bottom_2,
                                   grid.min = 0,
                                   grid.mid = 50,
                                   grid.max = 100,
                                   values.radar = c("", "", ""),
...
ggsave("sub_pro_4_sa_pl_analysis/images/20_21/sa_pl_analysis_20_21_perc_radar_top_bottom_2.png", height = 12, width = 12, dpi = 300)
