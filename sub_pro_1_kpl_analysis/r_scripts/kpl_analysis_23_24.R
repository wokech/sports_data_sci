# KPL League Table Analysis (2023-2024)
...
# 
# page_23_24 <- read_html("https://www.espn.com/soccer/standings/_/league/ken.1/season/2023")
# tables_23_24 <- html_table(page_23_24)
# kpl_1_23_24 <- tables_23_24[[1]]
# kpl_2_23_24 <- tables_23_24[[2]]
# 
# kpl_1_23_24 <- kpl_1_23_24 |>
#   rename(Club = `2023-24 KPL`)
# 
# kpl_1_23_24 <- kpl_1_23_24 |>
#   mutate(
...
#   select(-matches)
# 
# kpl_merge_23_24 <- bind_cols(kpl_1_23_24, kpl_2_23_24) 
# 
# kpl_merge_23_24 <- kpl_merge_23_24 |>
#   select(-Club)
# 
# # Save data as csv in datasets
# write_csv(kpl_merge_23_24, here::here("sub_pro_1_kpl_analysis", 
#                                       "datasets", "kpl_merge_23_24.csv"))

# Read in data
kpl_merge_23_24 <- read_csv(here::here("sub_pro_1_kpl_analysis", 
                                       "datasets", "kpl_merge_23_24.csv"))
...
kpl_merge_lollipop_23_24_w_l <- kpl_merge_23_24 |>
...
ggplot(kpl_merge_lollipop_23_24_w_l) +
...
kpl_merge_bar_23_24_ppg <- kpl_merge_23_24 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
  select(team_name, PPG) 

ggplot(kpl_merge_bar_23_24_ppg) +
...
kpl_merge_bar_23_24_gpg <- kpl_merge_23_24 |>
  mutate(GPG = round(F/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GPG) 

ggplot(kpl_merge_bar_23_24_gpg) +
...
kpl_merge_bar_23_24_gapg <- kpl_merge_23_24 |>
  mutate(GAPG = round(A/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GAPG) 

ggplot(kpl_merge_bar_23_24_gapg) +
...
kpl_merge_bar_23_24_gdpg <- kpl_merge_23_24 |>
  mutate(GDPG = round(GD/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GDPG) 

ggplot(kpl_merge_bar_23_24_gdpg) +
...
kpl_merge_bar_23_24_gf_ga <- kpl_merge_23_24 |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
  select(team_name, F, A)

# Thresholds
against_thresh <- median(kpl_merge_bar_23_24_gf_ga$A)
for_thresh <- median(kpl_merge_bar_23_24_gf_ga$F)

ggplot(kpl_merge_bar_23_24_gf_ga, aes(x = A, y = F)) +
...
kpl_merge_bar_23_24_ppg_gd <- kpl_merge_23_24 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
...
# Thresholds
goal_diff_thresh <- median(kpl_merge_bar_23_24_ppg_gd$GD)
ppg_thresh <- median(kpl_merge_bar_23_24_ppg_gd$PPG)

ggplot(kpl_merge_bar_23_24_ppg_gd, aes(x = GD, y = PPG)) +
...
kpl_merge_23_24_non_perc <- kpl_merge_23_24 |>
  mutate(
    PPG = P / GP,
    GPG = F / GP,
    GAPG = A / GP,
...
kpl_merge_23_24_perc <- kpl_merge_23_24 |>
  mutate(
    WinPerc = (W / GP) * 100,
    DrawPerc = (D / GP) * 100,
    LossPerc = (L / GP) * 100,
...
kpl_merge_23_24_non_perc_radar <- kpl_merge_23_24_non_perc |>
  select(team_name, PPG, GPG, GAPG, GF2GA, GDPG, GD2P) 

kpl_merge_23_24_perc_radar <- kpl_merge_23_24_perc |>
...
kpl_merge_23_24_perc_radar_top_3 <- kpl_merge_23_24_perc_radar |>
  filter(team_name == c("Gor Mahia", "Tusker", "Police"))

my_top_colors <- c("Gor Mahia" = "#BE8125", 
                   "Tusker" = "#2FBE25", 
                   "Police" = "#BE25AB")

perc_radar_top_3 <- ggradar(kpl_merge_23_24_perc_radar_top_3,
                            grid.min = 0,
                            grid.mid = 50,
                            grid.max = 100,
                            values.radar = c("", "", ""),
...
ggsave("sub_pro_1_kpl_analysis/images/23_24/kpl_analysis_23_24_perc_radar_top_3.png", height = 12, width = 12, dpi = 300)

# Bottom 3 Teams (Percent Metrics)

kpl_merge_23_24_perc_radar_bottom_3 <- kpl_merge_23_24_perc_radar |>
  filter(team_name == c("Sofapaka", "Muhoroni Youth", "Nzoia United"))

my_bottom_colors <- c("Sofapaka" = "#BE8125", 
                      "Muhoroni Youth" = "#2FBE25", 
                      "Nzoia United" = "#BE25AB")

perc_radar_bottom_3 <- ggradar(kpl_merge_23_24_perc_radar_bottom_3,
                               grid.min = 0,
                               grid.mid = 50,
                               grid.max = 100,
                               values.radar = c("", "", ""),
...
ggsave("sub_pro_1_kpl_analysis/images/23_24/kpl_analysis_23_24_perc_radar_bottom_3.png", height = 12, width = 12, dpi = 300)


# Top Bottom 2 Teams (Percent Metrics)

kpl_merge_23_24_perc_radar_top_bottom_2 <- kpl_merge_23_24_perc_radar |>
  filter(team_name %in% c("Gor Mahia", "Tusker", "Muhoroni Youth", "Nzoia United"))

my_top_bottom_colors <- c("Gor Mahia" = "#000080", 
                          "Tusker" = "#2FBE25", 
                          "Muhoroni Youth" = "#BE8125", 
                          "Nzoia United" = "#BE25AB")

perc_radar_top_bottom_2 <- ggradar(kpl_merge_23_24_perc_radar_top_bottom_2,
                                   grid.min = 0,
                                   grid.mid = 50,
                                   grid.max = 100,
                                   values.radar = c("", "", ""),
...
ggsave("sub_pro_1_kpl_analysis/images/23_24/kpl_analysis_23_24_perc_radar_top_bottom_2.png", height = 12, width = 12, dpi = 300)
