# KPL League Table Analysis (2022-2023)
...
# 
# page_22_23 <- read_html("https://www.espn.com/soccer/standings/_/league/ken.1/season/2022")
# tables_22_23 <- html_table(page_22_23)
# kpl_1_22_23 <- tables_22_23[[1]]
# kpl_2_22_23 <- tables_22_23[[2]]
# 
# kpl_1_22_23 <- kpl_1_22_23 |>
#   rename(Club = `2022-23 KPL`)
# 
# kpl_1_22_23 <- kpl_1_22_23 |>
#   mutate(
...
#   select(-matches)
# 
# kpl_merge_22_23 <- bind_cols(kpl_1_22_23, kpl_2_22_23) 
# 
# kpl_merge_22_23 <- kpl_merge_22_23 |>
#   select(-Club)
# 
# # Save data as csv in datasets
# write_csv(kpl_merge_22_23, here::here("sub_pro_1_kpl_analysis", 
#                                       "datasets", "kpl_merge_22_23.csv"))

# Read in data
kpl_merge_22_23 <- read_csv(here::here("sub_pro_1_kpl_analysis", 
                                       "datasets", "kpl_merge_22_23.csv"))
...
kpl_merge_lollipop_22_23_w_l <- kpl_merge_22_23 |>
...
ggplot(kpl_merge_lollipop_22_23_w_l) +
...
kpl_merge_bar_22_23_ppg <- kpl_merge_22_23 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
  select(team_name, PPG) 

ggplot(kpl_merge_bar_22_23_ppg) +
...
kpl_merge_bar_22_23_gpg <- kpl_merge_22_23 |>
  mutate(GPG = round(F/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GPG) 

ggplot(kpl_merge_bar_22_23_gpg) +
...
kpl_merge_bar_22_23_gapg <- kpl_merge_22_23 |>
  mutate(team_name = if_else(team_name == "CPosta Rangers",
                             "Posta Rangers", team_name)) |>
  mutate(team_name = if_else(team_name == "Bandari Mtwara",
                             "Bandari", team_name)) |>
...
ggplot(kpl_merge_bar_22_23_gapg) +
...
kpl_merge_bar_22_23_gdpg <- kpl_merge_22_23 |>
  mutate(team_name = if_else(team_name == "CPosta Rangers",
                             "Posta Rangers", team_name)) |>
  mutate(team_name = if_else(team_name == "Bandari Mtwara",
                             "Bandari", team_name)) |>
...
ggplot(kpl_merge_bar_22_23_gdpg) +
...
kpl_merge_bar_22_23_gf_ga <- kpl_merge_22_23 |>
  mutate(team_name = if_else(team_name == "CPosta Rangers",
                             "Posta Rangers", team_name)) |>
  mutate(team_name = if_else(team_name == "Bandari Mtwara",
                             "Bandari", team_name)) |>
...
# Thresholds
against_thresh <- median(kpl_merge_bar_22_23_gf_ga$A)
for_thresh <- median(kpl_merge_bar_22_23_gf_ga$F)

ggplot(kpl_merge_bar_22_23_gf_ga, aes(x = A, y = F)) +
...
kpl_merge_bar_22_23_ppg_gd <- kpl_merge_22_23 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
...
# Thresholds
goal_diff_thresh <- median(kpl_merge_bar_22_23_ppg_gd$GD)
ppg_thresh <- median(kpl_merge_bar_22_23_ppg_gd$PPG)

ggplot(kpl_merge_bar_22_23_ppg_gd, aes(x = GD, y = PPG)) +
...
kpl_merge_22_23_non_perc <- kpl_merge_22_23 |>
  mutate(
    PPG = P / GP,
    GPG = F / GP,
    GAPG = A / GP,
...
kpl_merge_22_23_perc <- kpl_merge_22_23 |>
  mutate(
    WinPerc = (W / GP) * 100,
    DrawPerc = (D / GP) * 100,
    LossPerc = (L / GP) * 100,
...
kpl_merge_22_23_non_perc_radar <- kpl_merge_22_23_non_perc |>
  select(team_name, PPG, GPG, GAPG, GF2GA, GDPG, GD2P) 

kpl_merge_22_23_perc_radar <- kpl_merge_22_23_perc |>
...
kpl_merge_22_23_perc_radar_top_3 <- kpl_merge_22_23_perc_radar |>
...
perc_radar_top_3 <- ggradar(kpl_merge_22_23_perc_radar_top_3,
                            grid.min = 0,
                            grid.mid = 50,
                            grid.max = 100,
                            values.radar = c("", "", ""),
...
kpl_merge_22_23_perc_radar_bottom_3 <- kpl_merge_22_23_perc_radar |>
  filter(team_name == c("Wazito", "Mathare United", "Vihiga Bullets"))

my_bottom_colors <- c("Wazito" = "#BE8125", 
                      "Mathare United" = "#2FBE25", 
                      "Vihiga Bullets" = "#BE25AB")

perc_radar_bottom_3 <- ggradar(kpl_merge_22_23_perc_radar_bottom_3,
                               grid.min = 0,
                               grid.mid = 50,
                               grid.max = 100,
                               values.radar = c("", "", ""),
...
kpl_merge_22_23_perc_radar_top_bottom_2 <- kpl_merge_22_23_perc_radar |>
  filter(team_name %in% c("Gor Mahia", "Tusker", "Mathare United", "Vihiga Bullets"))

my_top_bottom_colors <- c("Gor Mahia" = "#000080", 
                          "Tusker" = "#2FBE25", 
                          "Mathare United" = "#BE8125", 
                          "Vihiga Bullets" = "#BE25AB")

perc_radar_top_bottom_2 <- ggradar(kpl_merge_22_23_perc_radar_top_bottom_2,
                                   grid.min = 0,
                                   grid.mid = 50,
                                   grid.max = 100,
                                   values.radar = c("", "", ""),
