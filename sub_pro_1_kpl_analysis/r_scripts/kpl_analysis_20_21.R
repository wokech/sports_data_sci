...
# write_csv(kpl_merge_20_21, here::here("sub_pro_1_kpl_analysis",
#                                       "datasets", "kpl_merge_20_21.csv"))

# Read in data

kpl_merge_20_21 <- read_csv(here::here("sub_pro_1_kpl_analysis", 
                                       "datasets", "kpl_merge_20_21.csv"))
...
kpl_merge_bar_20_21_ppg <- kpl_merge_20_21 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
  select(team_name, PPG) 

ggplot(kpl_merge_bar_20_21_ppg) +
...
kpl_merge_bar_20_21_gpg <- kpl_merge_20_21 |>
  mutate(GPG = round(F/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GPG) 

ggplot(kpl_merge_bar_20_21_gpg) +
...
kpl_merge_bar_20_21_gapg <- kpl_merge_20_21 |>
  mutate(GAPG = round(A/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GAPG) 

ggplot(kpl_merge_bar_20_21_gapg) +
...
kpl_merge_bar_20_21_gdpg <- kpl_merge_20_21 |>
  mutate(GDPG = round(GD/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GDPG) 

ggplot(kpl_merge_bar_20_21_gdpg) +
...
kpl_merge_bar_20_21_gf_ga <- kpl_merge_20_21 |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
  select(team_name, F, A)

# Thresholds
against_thresh <- median(kpl_merge_bar_20_21_gf_ga$A)
for_thresh <- median(kpl_merge_bar_20_21_gf_ga$F)

ggplot(kpl_merge_bar_20_21_gf_ga, aes(x = A, y = F)) +
...
kpl_merge_bar_20_21_ppg_gd <- kpl_merge_20_21 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
...
# Thresholds
goal_diff_thresh <- median(kpl_merge_bar_20_21_ppg_gd$GD, na.rm = TRUE)
ppg_thresh <- median(kpl_merge_bar_20_21_ppg_gd$PPG, na.rm = TRUE)

ggplot(kpl_merge_bar_20_21_ppg_gd, aes(x = GD, y = PPG)) +
...
kpl_merge_20_21_non_perc <- kpl_merge_20_21 |>
  mutate(
    PPG = P / GP,
    GPG = F / GP,
    GAPG = A / GP,
...
kpl_merge_20_21_perc <- kpl_merge_20_21 |>
  mutate(
    WinPerc = (W / GP) * 100,
    DrawPerc = (D / GP) * 100,
    LossPerc = (L / GP) * 100,
...
kpl_merge_20_21_non_perc_radar <- kpl_merge_20_21_non_perc |>
  select(team_name, PPG, GPG, GAPG, GF2GA, GDPG, GD2P) 

kpl_merge_20_21_perc_radar <- kpl_merge_20_21_perc |>
...
kpl_merge_20_21_perc_radar_top_3 <- kpl_merge_20_21_perc_radar |>
  filter(team_name %in% c("Tusker", "KCB", "Bandari"))

my_top_colors <- c("Tusker" = "#BE8125", 
                   "KCB" = "#2FBE25", 
                   "Bandari" = "#BE25AB")

perc_radar_top_3 <- ggradar(kpl_merge_20_21_perc_radar_top_3,
                            grid.min = 0,
                            grid.mid = 50,
                            grid.max = 100,
                            values.radar = c("", "", ""),
...
kpl_merge_20_21_perc_radar_bottom_3 <- kpl_merge_20_21_perc_radar |>
  filter(team_name %in% c("Mathare United", "Vihiga United", "Western Stima"))

my_bottom_colors <- c("Mathare United" = "#BE8125", 
                      "Vihiga United" = "#2FBE25", 
                      "Western Stima" = "#BE25AB")

perc_radar_bottom_3 <- ggradar(kpl_merge_20_21_perc_radar_bottom_3,
                               grid.min = 0,
                               grid.mid = 50,
                               grid.max = 100,
                               values.radar = c("", "", ""),
...
kpl_merge_20_21_perc_radar_top_bottom_2 <- kpl_merge_20_21_perc_radar |>
  filter(team_name %in% c("Tusker", "KCB", "Vihiga United", "Western Stima"))

my_top_bottom_colors <- c("Tusker" = "#000080", 
                          "KCB" = "#2FBE25", 
                          "Vihiga United" = "#BE8125", 
                          "Western Stima" = "#BE25AB")

perc_radar_top_bottom_2 <- ggradar(kpl_merge_20_21_perc_radar_top_bottom_2,
                                   grid.min = 0,
                                   grid.mid = 50,
                                   grid.max = 100,
                                   values.radar = c("", "", ""),
