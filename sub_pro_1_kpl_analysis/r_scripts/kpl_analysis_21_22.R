# KPL League Table Analysis (2021-2022)
...
# page_21_22 <- read_html("https://www.espn.com/soccer/standings/_/league/ken.1/season/2021")
# tables_21_22 <- html_table(page_21_22)
# kpl_1_21_22 <- tables_21_22[[1]]
# kpl_2_21_22 <- tables_21_22[[2]]
# 
# kpl_1_21_22 <- kpl_1_21_22 |>
...
#   select(-matches)
# 
# kpl_merge_21_22 <- bind_cols(kpl_1_21_22, kpl_2_21_22)
# 
# kpl_merge_21_22 <- kpl_merge_21_22 |>
#   select(-Club)
# 
# # Save data as csv in datasets
# write_csv(kpl_merge_21_22, here::here("sub_pro_1_kpl_analysis",
#                                       "datasets", "kpl_merge_21_22.csv"))

# Read in data

kpl_merge_21_22 <- read_csv(here::here("sub_pro_1_kpl_analysis", 
                                       "datasets", "kpl_merge_21_22.csv"))
...
ggsave("sub_pro_1_kpl_analysis/images/21_22/kpl_analysis_21_22_pt_gd.png", width = 12, height = 12, dpi = 300)


# 2) Plot of Wins and Losses in a Lollipop Chart

kpl_merge_lollipop_21_22_w_l <- kpl_merge_21_22 |>
...
ggplot(kpl_merge_lollipop_21_22_w_l) +
...
ggsave("sub_pro_1_kpl_analysis/images/21_22/kpl_analysis_21_22_w_l.png", width = 12, height = 12, dpi = 300)

# 3) Plot of Points per Goal

kpl_merge_bar_21_22_ppg <- kpl_merge_21_22 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
  select(team_name, PPG) 

ggplot(kpl_merge_bar_21_22_ppg) +
...
ggsave("sub_pro_1_kpl_analysis/images/21_22/kpl_analysis_21_22_ppg.png", width = 12, height = 12, dpi = 300)

# 4) Plot of Goals per Game

kpl_merge_bar_21_22_gpg <- kpl_merge_21_22 |>
  mutate(GPG = round(F/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GPG) 

ggplot(kpl_merge_bar_21_22_gpg) +
...
ggsave("sub_pro_1_kpl_analysis/images/21_22/kpl_analysis_21_22_gpg.png", width = 12, height = 12, dpi = 300)

# 5) Plot of Goals Against per Game

kpl_merge_bar_21_22_gapg <- kpl_merge_21_22 |>
  mutate(GAPG = round(A/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GAPG) 

ggplot(kpl_merge_bar_21_22_gapg) +
...
ggsave("sub_pro_1_kpl_analysis/images/21_22/kpl_analysis_21_22_ga_pg.png", width = 12, height = 12, dpi = 300)


# 6) Plot of Goal Difference per Game

kpl_merge_bar_21_22_gdpg <- kpl_merge_21_22 |>
  mutate(GDPG = round(GD/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GDPG) 

ggplot(kpl_merge_bar_21_22_gdpg) +
...
ggsave("sub_pro_1_kpl_analysis/images/21_22/kpl_analysis_21_22_gd_pg.png", width = 12, height = 12, dpi = 300)

# 7) Scatterplot of GF vs GA

kpl_merge_bar_21_22_gf_ga <- kpl_merge_21_22 |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
  select(team_name, F, A)

# Thresholds
against_thresh <- median(kpl_merge_bar_21_22_gf_ga$A)
for_thresh <- median(kpl_merge_bar_21_22_gf_ga$F)

ggplot(kpl_merge_bar_21_22_gf_ga, aes(x = A, y = F)) +
...
ggsave("sub_pro_1_kpl_analysis/images/21_22/kpl_analysis_21_22_gf_ga.png", width = 12, height = 12, dpi = 300)


# 8) Scatterplot of PPG vs GD

kpl_merge_bar_21_22_ppg_gd <- kpl_merge_21_22 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
...
# Thresholds
goal_diff_thresh <- median(kpl_merge_bar_21_22_ppg_gd$GD)
ppg_thresh <- median(kpl_merge_bar_21_22_ppg_gd$PPG)

ggplot(kpl_merge_bar_21_22_ppg_gd, aes(x = GD, y = PPG)) +
...
ggsave("sub_pro_1_kpl_analysis/images/21_22/kpl_analysis_21_22_ppg_gd.png", width = 12, height = 12, dpi = 300)

# 9) Radar Chart - Sample teams to plot

# a) Non-Percentage metrics

kpl_merge_21_22_non_perc <- kpl_merge_21_22 |>
  mutate(
    PPG = P / GP,
    GPG = F / GP,
    GAPG = A / GP,
...
kpl_merge_21_22_perc <- kpl_merge_21_22 |>
  mutate(
    WinPerc = (W / GP) * 100,
    DrawPerc = (D / GP) * 100,
    LossPerc = (L / GP) * 100,
...
kpl_merge_21_22_non_perc_radar <- kpl_merge_21_22_non_perc |>
  select(team_name, PPG, GPG, GAPG, GF2GA, GDPG, GD2P) 

kpl_merge_21_22_perc_radar <- kpl_merge_21_22_perc |>
...
kpl_merge_21_22_perc_radar_top_3 <- kpl_merge_21_22_perc_radar |>
  filter(team_name %in% c("Tusker", "Kakamega Homeboyz", "Bandari"))

my_top_colors <- c("Tusker" = "#BE8125", 
                   "Kakamega Homeboyz" = "#2FBE25", 
                   "Bandari" = "#BE25AB")

perc_radar_top_3 <- ggradar(kpl_merge_21_22_perc_radar_top_3,
                            grid.min = 0,
                            grid.mid = 50,
                            grid.max = 100,
                            values.radar = c("", "", ""),
...
ggsave("sub_pro_1_kpl_analysis/images/21_22/kpl_analysis_21_22_perc_radar_top_3.png", height = 12, width = 12, dpi = 300)


# Bottom 3 Teams (Percent Metrics)

kpl_merge_21_22_perc_radar_bottom_3 <- kpl_merge_21_22_perc_radar |>
  filter(team_name %in% c("Wazito", "Vihiga Bullets", "Mathare United"))

my_bottom_colors <- c("Wazito" = "#BE8125", 
                      "Vihiga Bullets" = "#2FBE25", 
                      "Mathare United" = "#BE25AB")

perc_radar_bottom_3 <- ggradar(kpl_merge_21_22_perc_radar_bottom_3,
                               grid.min = 0,
                               grid.mid = 50,
                               grid.max = 100,
                               values.radar = c("", "", ""),
...
ggsave("sub_pro_1_kpl_analysis/images/21_22/kpl_analysis_21_22_perc_radar_bottom_3.png", height = 12, width = 12, dpi = 300)


# Top Bottom 2 Teams (Percent Metrics)

kpl_merge_21_22_perc_radar_top_bottom_2 <- kpl_merge_21_22_perc_radar |>
  filter(team_name %in% c("Tusker", "Kakamega Homeboyz", "Vihiga Bullets", "Mathare United"))

my_top_bottom_colors <- c("Tusker" = "#000080", 
                          "Kakamega Homeboyz" = "#2FBE25", 
                          "Vihiga Bullets" = "#BE8125", 
                          "Mathare United" = "#BE25AB")

perc_radar_top_bottom_2 <- ggradar(kpl_merge_21_22_perc_radar_top_bottom_2,
                                   grid.min = 0,
                                   grid.mid = 50,
                                   grid.max = 100,
                                   values.radar = c("", "", ""),
...
ggsave("sub_pro_1_kpl_analysis/images/21_22/kpl_analysis_21_22_perc_radar_top_bottom_2.png", height = 12, width = 12, dpi = 300)
