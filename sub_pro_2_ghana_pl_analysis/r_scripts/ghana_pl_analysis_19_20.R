# Ghana PL League Table Analysis (2019-2020)
...
# 
# page_19_20 <- read_html("https://www.espn.com/soccer/standings/_/league/GHA.1/season/2019")
# tables_19_20 <- html_table(page_19_20)
# ghana_pl_1_19_20 <- tables_19_20[[1]]
# ghana_pl_2_19_20 <- tables_19_20[[2]]
# 
# ghana_pl_1_19_20 <- ghana_pl_1_19_20 |>
...
#   select(-matches) 
# 
# ghana_pl_merge_19_20 <- bind_cols(ghana_pl_1_19_20, ghana_pl_2_19_20) |>
#   mutate(team_name = replace(team_name, team_name == "GKing Faisal Babes", "King Faisal Babes"))
# 
# ghana_pl_merge_19_20 <- ghana_pl_merge_19_20 |>
#   select(-Club)
# 
# # Save data as csv in datasets
# write_csv(ghana_pl_merge_19_20, here::here("sub_pro_2_ghana_pl_analysis",
#                                      "datasets", "ghana_pl_merge_19_20.csv"))

# Read in data
ghana_pl_merge_19_20 <- read_csv(here::here("sub_pro_2_ghana_pl_analysis", 
                                       "datasets", "ghana_pl_merge_19_20.csv"))

# Display the structure of the data
str(ghana_pl_merge_19_20)
head(ghana_pl_merge_19_20)

# 1) Plot of Points and Goal Differences in a Bar Chart

ghana_pl_merge_bar_19_20_pt_gd <- ghana_pl_merge_19_20 |>
  select(team_name, GD, P)

team_order <- ghana_pl_merge_bar_19_20_pt_gd %>%
  arrange(desc(P), desc(GD)) %>%
  pull(team_name)

ghana_pl_merge_bar_19_20_pt_gd_long <- ghana_pl_merge_bar_19_20_pt_gd |>
  pivot_longer(cols = c(P, GD), names_to = "metric", values_to = "value")

# Plot

ghana_pl_merge_bar_19_20_pt_gd_long %>%
  mutate(team_name = factor(team_name, levels = rev(team_order))) %>%
  ggplot(aes(x = team_name, y = value, fill = metric)) +
  geom_col(position = position_dodge()) +
  geom_text(data = ghana_pl_merge_bar_19_20_pt_gd,
...
ggsave("sub_pro_2_ghana_pl_analysis/images/19_20/ghana_pl_analysis_19_20_pt_gd.png", width = 12, height = 12, dpi = 300)


# 2) Plot of Wins and Losses in a Lollipop Chart

ghana_pl_merge_lollipop_19_20_w_l <- ghana_pl_merge_19_20 |>
...
ggplot(ghana_pl_merge_lollipop_19_20_w_l) +
...
ggsave("sub_pro_2_ghana_pl_analysis/images/19_20/ghana_pl_analysis_19_20_w_l.png", width = 12, height = 12, dpi = 300)

# 3) Plot of Points per Goal

ghana_pl_merge_bar_19_20_ppg <- ghana_pl_merge_19_20 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
  select(team_name, PPG) 

ggplot(ghana_pl_merge_bar_19_20_ppg) +
...
ggsave("sub_pro_2_ghana_pl_analysis/images/19_20/ghana_pl_analysis_19_20_ppg.png", width = 12, height = 12, dpi = 300)

# 4) Plot of Goals per Game

ghana_pl_merge_bar_19_20_gpg <- ghana_pl_merge_19_20 |>
  mutate(GPG = round(F/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GPG) 

ggplot(ghana_pl_merge_bar_19_20_gpg) +
...
ggsave("sub_pro_2_ghana_pl_analysis/images/19_20/ghana_pl_analysis_19_20_gpg.png", width = 12, height = 12, dpi = 300)


# 5) Plot of Goals Against per Game

ghana_pl_merge_bar_19_20_gapg <- ghana_pl_merge_19_20 |>
  mutate(GAPG = round(A/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GAPG) 

ggplot(ghana_pl_merge_bar_19_20_gapg) +
...
ggsave("sub_pro_2_ghana_pl_analysis/images/19_20/ghana_pl_analysis_19_20_ga_pg.png", width = 12, height = 12, dpi = 300)


# 6) Plot of Goal Difference per Game

ghana_pl_merge_bar_19_20_gdpg <- ghana_pl_merge_19_20 |>
  mutate(GDPG = round(GD/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GDPG) 

ggplot(ghana_pl_merge_bar_19_20_gdpg) +
...
ggsave("sub_pro_2_ghana_pl_analysis/images/19_20/ghana_pl_analysis_19_20_gd_pg.png", width = 12, height = 12, dpi = 300)

# 7) Scatterplot of GF vs GA

ghana_pl_merge_bar_19_20_gf_ga <- ghana_pl_merge_19_20 |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
  select(team_name, F, A)

# Thresholds
against_thresh <- median(ghana_pl_merge_bar_19_20_gf_ga$A)
for_thresh <- median(ghana_pl_merge_bar_19_20_gf_ga$F)

ggplot(ghana_pl_merge_bar_19_20_gf_ga, aes(x = A, y = F)) +
...
ggsave("sub_pro_2_ghana_pl_analysis/images/19_20/ghana_pl_analysis_19_20_gf_ga.png", width = 12, height = 12, dpi = 300)


# 8) Scatterplot of PPG vs GD

ghana_pl_merge_bar_19_20_ppg_gd <- ghana_pl_merge_19_20 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
...
# Thresholds
goal_diff_thresh <- median(ghana_pl_merge_bar_19_20_ppg_gd$GD)
ppg_thresh <- median(ghana_pl_merge_bar_19_20_ppg_gd$PPG)

ggplot(ghana_pl_merge_bar_19_20_ppg_gd, aes(x = GD, y = PPG)) +
  geom_point(color = "brown4", size = 6) +
  geom_text_repel(aes(label = team_name), vjust = -0.5, size = 8) +
...
ggsave("sub_pro_2_ghana_pl_analysis/images/19_20/ghana_pl_analysis_19_20_ppg_gd.png", width = 12, height = 12, dpi = 300)
...
ghana_pl_merge_19_20_non_perc <- ghana_pl_merge_19_20 |>
  mutate(
    PPG = P / GP,
    GPG = F / GP,
    GAPG = A / GP,
...
ghana_pl_merge_19_20_perc <- ghana_pl_merge_19_20 |>
  mutate(
    WinPerc = (W / GP) * 100,
    DrawPerc = (D / GP) * 100,
    LossPerc = (L / GP) * 100,
...
ghana_pl_merge_19_20_non_perc_radar <- ghana_pl_merge_19_20_non_perc |>
  select(team_name, PPG, GPG, GAPG, GF2GA, GDPG, GD2P) 

ghana_pl_merge_19_20_perc_radar <- ghana_pl_merge_19_20_perc |>
...
ggsave("sub_pro_2_ghana_pl_analysis/images/19_20/ghana_pl_analysis_19_20_perc_radar_top_3.png", height = 12, width = 12, dpi = 300)
...
ggsave("sub_pro_2_ghana_pl_analysis/images/19_20/ghana_pl_analysis_19_20_perc_radar_bottom_3.png", height = 12, width = 12, dpi = 300)
...
ggsave("sub_pro_2_ghana_pl_analysis/images/19_20/ghana_pl_analysis_19_20_perc_radar_top_bottom_2.png", height = 12, width = 12, dpi = 300)
