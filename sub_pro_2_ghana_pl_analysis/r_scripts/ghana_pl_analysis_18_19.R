# Ghana PL League Table Analysis (2018-2019)
...
# 
# page_18_19 <- read_html("https://www.espn.com/soccer/standings/_/league/GHA.1/season/2018")
# tables_18_19 <- html_table(page_18_19)
# ghana_pl_1_18_19 <- tables_18_19[[1]]
# ghana_pl_2_18_19 <- tables_18_19[[2]]
# 
# ghana_pl_1_18_19 <- ghana_pl_1_18_19 |>
...
#   select(-matches)
# 
# ghana_pl_merge_18_19 <- bind_cols(ghana_pl_1_18_19, ghana_pl_2_18_19)
# 
# ghana_pl_merge_18_19 <- ghana_pl_merge_18_19 |>
#   select(-Club)
# 
# # Save data as csv in datasets
# write_csv(ghana_pl_merge_18_19, here::here("sub_pro_2_ghana_pl_analysis",
#                                      "datasets", "ghana_pl_merge_18_19.csv"))

# Read in data
ghana_pl_merge_18_19 <- read_csv(here::here("sub_pro_2_ghana_pl_analysis", 
                                       "datasets", "ghana_pl_merge_18_19.csv"))

# Display the structure of the data
str(ghana_pl_merge_18_19)
head(ghana_pl_merge_18_19)

# 1) Plot of Points and Goal Differences in a Bar Chart

ghana_pl_merge_bar_18_19_pt_gd <- ghana_pl_merge_18_19 |>
  select(team_name, GD, P)

team_order <- ghana_pl_merge_bar_18_19_pt_gd %>%
  arrange(desc(P), desc(GD)) %>%
  pull(team_name)

ghana_pl_merge_bar_18_19_pt_gd_long <- ghana_pl_merge_bar_18_19_pt_gd |>
  pivot_longer(cols = c(P, GD), names_to = "metric", values_to = "value")

# Plot

ghana_pl_merge_bar_18_19_pt_gd_long %>%
  mutate(team_name = factor(team_name, levels = rev(team_order))) %>%
  ggplot(aes(x = team_name, y = value, fill = metric)) +
  geom_col(position = position_dodge()) +
  geom_text(data = ghana_pl_merge_bar_18_19_pt_gd,
...
ggsave("sub_pro_2_ghana_pl_analysis/images/18_19/ghana_pl_analysis_18_19_pt_gd.png", width = 12, height = 12, dpi = 300)


# 2) Plot of Wins and Losses in a Lollipop Chart

ghana_pl_merge_lollipop_18_19_w_l <- ghana_pl_merge_18_19 |>
...
ggplot(ghana_pl_merge_lollipop_18_19_w_l) +
...
ggsave("sub_pro_2_ghana_pl_analysis/images/18_19/ghana_pl_analysis_18_19_w_l.png", width = 12, height = 12, dpi = 300)

# 3) Plot of Points per Goal

ghana_pl_merge_bar_18_19_ppg <- ghana_pl_merge_18_19 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
  select(team_name, PPG) 

ggplot(ghana_pl_merge_bar_18_19_ppg) +
...
ggsave("sub_pro_2_ghana_pl_analysis/images/18_19/ghana_pl_analysis_18_19_ppg.png", width = 12, height = 12, dpi = 300)

# 4) Plot of Goals per Game

ghana_pl_merge_bar_18_19_gpg <- ghana_pl_merge_18_19 |>
  mutate(GPG = round(F/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GPG) 

ggplot(ghana_pl_merge_bar_18_19_gpg) +
...
ggsave("sub_pro_2_ghana_pl_analysis/images/18_19/ghana_pl_analysis_18_19_gpg.png", width = 12, height = 12, dpi = 300)


# 5) Plot of Goals Against per Game

ghana_pl_merge_bar_18_19_gapg <- ghana_pl_merge_18_19 |>
  mutate(GAPG = round(A/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GAPG) 

ggplot(ghana_pl_merge_bar_18_19_gapg) +
...
ggsave("sub_pro_2_ghana_pl_analysis/images/18_19/ghana_pl_analysis_18_19_ga_pg.png", width = 12, height = 12, dpi = 300)


# 6) Plot of Goal Difference per Game

ghana_pl_merge_bar_18_19_gdpg <- ghana_pl_merge_18_19 |>
  mutate(GDPG = round(GD/GP, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column
  select(team_name, GDPG) 

ggplot(ghana_pl_merge_bar_18_19_gdpg) +
...
ggsave("sub_pro_2_ghana_pl_analysis/images/18_19/ghana_pl_analysis_18_19_gd_pg.png", width = 12, height = 12, dpi = 300)

# 7) Scatterplot of GF vs GA

ghana_pl_merge_bar_18_19_gf_ga <- ghana_pl_merge_18_19 |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
  select(team_name, F, A)

# Thresholds
against_thresh <- median(ghana_pl_merge_bar_18_19_gf_ga$A)
for_thresh <- median(ghana_pl_merge_bar_18_19_gf_ga$F)

ggplot(ghana_pl_merge_bar_18_19_gf_ga, aes(x = A, y = F)) +
...
ggsave("sub_pro_2_ghana_pl_analysis/images/18_19/ghana_pl_analysis_18_19_gf_ga.png", width = 12, height = 12, dpi = 300)


# 8) Scatterplot of PPG vs GD

ghana_pl_merge_bar_18_19_ppg_gd <- ghana_pl_merge_18_19 |>
  mutate(PPG = round(P/F, 3)) |>
  mutate(team_name = fct_reorder(team_name, GD)) |> # Order by tie-breaker
  mutate(team_name = fct_reorder(team_name, P)) |> # Order by main column       
...
# Thresholds
goal_diff_thresh <- median(ghana_pl_merge_bar_18_19_ppg_gd$GD)
ppg_thresh <- median(ghana_pl_merge_bar_18_19_ppg_gd$PPG)

ggplot(ghana_pl_merge_bar_18_19_ppg_gd, aes(x = GD, y = PPG)) +
  geom_point(color = "brown4", size = 6) +
  geom_text_repel(aes(label = team_name), vjust = -0.5, size = 8) +
...
ggsave("sub_pro_2_ghana_pl_analysis/images/18_19/ghana_pl_analysis_18_19_ppg_gd.png", width = 12, height = 12, dpi = 300)
...
ghana_pl_merge_18_19_non_perc <- ghana_pl_merge_18_19 |>
  mutate(
    PPG = P / GP,
    GPG = F / GP,
    GAPG = A / GP,
...
ghana_pl_merge_18_19_perc <- ghana_pl_merge_18_19 |>
  mutate(
    WinPerc = (W / GP) * 100,
    DrawPerc = (D / GP) * 100,
    LossPerc = (L / GP) * 100,
...
ghana_pl_merge_18_19_non_perc_radar <- ghana_pl_merge_18_19_non_perc |>
  select(team_name, PPG, GPG, GAPG, GF2GA, GDPG, GD2P) 

ghana_pl_merge_18_19_perc_radar <- ghana_pl_merge_18_19_perc |>
...
ghana_pl_merge_18_19_perc_radar_top_3 <- ghana_pl_merge_18_19_perc_radar |>
  filter(team_name == c("Medeama", "Ashanti Gold", "Karela"))

my_top_colors <- c("Medeama" = "#BE8125", 
                   "Ashanti Gold" = "#2FBE25", 
                   "Karela" = "#BE25AB")

perc_radar_top_3 <- ggradar(ghana_pl_merge_18_19_perc_radar_top_3,
                            grid.min = 0,
                            grid.mid = 50,
                            grid.max = 100,
                            values.radar = c("", "", ""),
...
ggsave("sub_pro_2_ghana_pl_analysis/images/18_19/ghana_pl_analysis_18_19_perc_radar_top_3.png", height = 12, width = 12, dpi = 300)


# Bottom 3 Teams (Percent Metrics)

ghana_pl_merge_18_19_perc_radar_bottom_3 <- ghana_pl_merge_18_19_perc_radar |>
  filter(team_name %in% c("Aduana Stars", "Ebusua Dwarfs", "Legon Cities"))

my_bottom_colors <- c("Aduana Stars" = "#BE8125", 
                      "Ebusua Dwarfs" = "#2FBE25", 
                      "Legon Cities" = "#BE25AB")

perc_radar_bottom_3 <- ggradar(ghana_pl_merge_18_19_perc_radar_bottom_3,
                               grid.min = 0,
                               grid.mid = 50,
                               grid.max = 100,
                               values.radar = c("", "", ""),
...
ggsave("sub_pro_2_ghana_pl_analysis/images/18_19/ghana_pl_analysis_18_19_perc_radar_bottom_3.png", height = 12, width = 12, dpi = 300)


# Bottom 3 Teams (Percent Metrics)

ghana_pl_merge_18_19_perc_radar_top_bottom_2 <- ghana_pl_merge_18_19_perc_radar |>
  filter(team_name %in% c("Medeama", "Ashanti Gold", "Ebusua Dwarfs", "Legon Cities"))

my_top_bottom_colors <- c("Medeama" = "#000080", 
                          "Ashanti Gold" = "#2FBE25", 
                          "Ebusua Dwarfs" = "#BE8125", 
                          "Legon Cities" = "#BE25AB")

perc_radar_top_bottom_2 <- ggradar(ghana_pl_merge_18_19_perc_radar_top_bottom_2,
                                   grid.min = 0,
                                   grid.mid = 50,
                                   grid.max = 100,
                                   values.radar = c("", "", ""),
...
ggsave("sub_pro_2_ghana_pl_analysis/images/18_19/ghana_pl_analysis_18_19_perc_radar_top_bottom_2.png", height = 12, width = 12, dpi = 300)
