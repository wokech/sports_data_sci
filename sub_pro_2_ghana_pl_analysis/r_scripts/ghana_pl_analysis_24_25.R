...
ghana_pl_merge_bar_24_25_pt_gd_long <- ghana_pl_merge_bar_24_25_pt_gd %>%
  ...
  mutate(team_name = fct_reorder(team_name, ghana_pl_merge_bar_24_25_pt_gd$P[match(team_name, ghana_pl_merge_bar_24_25_pt_gd$team_name)]))

# Plot
ggplot(ghana_pl_merge_bar_24_25_pt_gd_long, aes(x = team_name, y = value, fill = metric)) +
...
ghana_pl_radar <- ghana_pl_merge_24_25 %>%
  mutate(
...
create_radar_chart <- function(team_indices, title = "ghana_pl Teams Performance Radar") {
  ...
  radar_data <- ghana_pl_radar[team_indices, metrics]
  ...
  # Add legend
  legend(x = 0.8, y = 1.2, 
         legend = ghana_pl_radar$team_name[team_indices], 
...
top_4 <- 1:4
print(ghana_pl_radar[top_4, c("team_name", "P", "Win_Pct", "Attacking_Strength", "Defensive_Strength")])

par(mfrow = c(1, 1), mar = c(1, 1, 3, 1))
create_radar_chart(top_4, "ghana_pl Top 4 Teams - Performance Radar")
...
print(ghana_pl_radar[style_comparison, c("team_name", "F", "A", "GD", "Win_Pct")])

par(mfrow = c(1, 1), mar = c(1, 1, 3, 1))
create_radar_chart(style_comparison, "ghana_pl Teams - Different Playing Styles")

# Example 3: Bottom vs Top teams comparison
cat("\n=== TOP vs BOTTOM TEAMS ===\n")
top_bottom <- c(1, 2, 17, 18)  # Police, Gor Mahia vs Talanta, Nairobi City Stars
print(ghana_pl_radar[top_bottom, c("team_name", "P", "F", "A", "GD")])

par(mfrow = c(1, 1), mar = c(1, 1, 3, 1))
create_radar_chart(top_bottom, "ghana_pl - Top Teams vs Bottom Teams")

# Display summary statistics
cat("\n=== LEAGUE SUMMARY STATISTICS ===\n")
cat("Champions:", ghana_pl_merge_24_25$team_name[1], "with", ghana_pl_merge_24_25$P[1], "points\n")
cat("Highest scoring team:", ghana_pl_merge_24_25$team_name[which.max(ghana_pl_merge_24_25$F)], "with", max(ghana_pl_merge_24_25$F), "goals\n")
cat("Best defense:", ghana_pl_merge_24_25$team_name[which.min(ghana_pl_merge_24_25$A)], "with", min(ghana_pl_merge_24_25$A), "goals conceded\n")
cat("Best goal difference:", ghana_pl_merge_24_25$team_name[which.max(ghana_pl_merge_24_25$GD)], "with", max(ghana_pl_merge_24_25$GD), "GD\n")

# Create a comprehensive performance table
performance_summary <- ghana_pl_radar %>%
